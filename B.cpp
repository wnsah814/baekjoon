    ...
    for (int gr = 0; gr < mGradeCnt; ++gr) {
        ...
        if (mGenderCnt == 2) {
            for (int gender = 0; gender < 2; ++gender) {
                if (db[grade][gender].size() == 0) continue;
                sort(db[grade][gender].begin(), db[grade][gender].end(), cmp_qry);
                auto it = lower_bound(db[grade][gender].begin(), db[grade][gender].end(), pi(mScore, 0));
                if (it == db[grade][gender].end()) continue;
                auto idx = it - db[grade][gender].begin();
                if (db[grade][gender][idx].first < mScore) continue;
                if (idx >= 0) {
                    ans.push_back(db[grade][gender][idx]);
                }
            }
        } else {
